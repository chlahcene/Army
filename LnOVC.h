...
{
    char tab[Taille_bloc_L7OVC+1];  // le tableau de caractère
...
typedef struct Entete_L7OVC        // structure de l'entete du fichier en memoire centrale
{
    int nbbloc;
    int tete;
    int queue;
...
//-------------------------------------permet d'allouer un nouveau bloc dans le ficiher et de maettre a jour les caracteristiques des bloc eet de l'entete touches------------------------------//
void  alloc_bloc_L7OVC(L7OVC *fichier);
//----------------------------------- // procedure qui permet d'ecrire n caractère du buffer----------------------------------------------//
void ecrire_chaine_enrg(L7OVC *fichier,int *i, int *j, char chaine[],Buffer_L7OVC *buf);
//----------------------------------- fonction qui permet de recuperer une chaine de longueur n dans le bloc i  lu dans buffer a partir de la position j-------------//
void recuperer_chaine_enrg(L7OVC *fichier,int *i, int *j, char chaine[],Buffer_L7OVC *buf); //
...
void aff_entete_L7OVC(L7OVC *fichier,int i , int valeur){
    ...
    switch(i)
    {
         ...
         case 4:
        {
         ...
         case 5:
        {
            fichier->entete.nb_car_sup=valeur;  // nombre de caractères suprime depuis la
...
void liredir_L7OVC(L7OVC *fichier, int i , Buffer_L7OVC *buf)
{
    ...
    fseek(fichier->F,(sizeof(Entete_L7OVC)+sizeof(Tbloc_L7OVC)*(i-1)),SEEK_SET); // positionnement au debut du bloc numero i
    fread(buf,sizeof(Buffer_L7OVC),1,fichier->F);                         //lecture d'un bloc de caractère correspondant a la taille du bloc dans le buffer
...
void ecriredir_L7OVC(L7OVC *fichier, int i, Buffer_L7OVC *buf)
{
    ecrire_stat_lonvc++;
     ...
     fseek(fichier->F,sizeof(Entete_L7OVC)+sizeof(Tbloc_L7OVC)*(i-1),SEEK_SET); // positionnement au debut du bloc numero i
     fwrite(buf,sizeof(Buffer_L7OVC),1,fichier->F);                       //ecriture du contenu du buffer dans le bloc numero i du fichier
...
L7OVC *ouvrir_L7OVC(char nom_physique[], char mode)
{
    ...
    if( (mode=='A') ||  (mode == 'a') )                        // mode ancien
    {
        ...
        fread(&(fichier->entete),sizeof(Entete_L7OVC),1,fichier->F);  // chargement de l'entete enregistree en debut de fichier
    }
    else
    {
        if( (mode=='N') ||  (mode == 'n') )                   // mode nouveau
        {
             ...
             aff_entete_L7OVC(fichier,1,0);                         // mise a zeo du nombre de bloc si entete(fichier,1)=0 alors le fichier est vide
             aff_entete_L7OVC(fichier,2,1);                         // mettre tete au premier bloc
             aff_entete_L7OVC(fichier,3,1);                         // de meme pour la queue puisque au debut tete est queue sont confondues
             aff_entete_L7OVC(fichier,4,0);                         // le premier caractère du ficheir correspond a laposition libre puisqu'il est nouveau
             aff_entete_L7OVC(fichier,5,0);                         // aucun caractère n'a encore ete supprime
             fwrite(&(fichier->entete),sizeof(Entete_L7OVC),1,fichier->F);// enregistrement de l'entete dans le fichier
             buf.suivant=-1;                                 // le suivant du premier bloc a NULL
             sprintf(buf.tab,"%s","");                       // initialisation du buffer a chaine vide
             ecriredir_L7OVC(fichier,1,&buf);                      // ecriture du premier bloc dans le fichier
...
void fermer_L7OVC(L7OVC *fichier)  // procedure de fermeture du fichier
{
    ...
    fwrite(&(fichier->entete),sizeof(Entete_L7OVC),1,fichier->F); // sauvegarde de la dernière version de l'entete de la strucuture L7OVC
                                                            //dans le fichier
    rewind(fichier->F);// repositionnement du curseur en debut de fichier
...
void  alloc_bloc_L7OVC(L7OVC *fichier)
{
        ...
        liredir_L7OVC(fichier,entete_L7OVC(fichier,3),buf);   // lecture du bloc correspondant a la queue
        buf->suivant=entete_L7OVC(fichier,1)+1;         // mise a jour dui suvant de la queue au bloc correspondant a la nouvelle queue
        ecriredir_L7OVC(fichier,entete_L7OVC(fichier,3),buf);// ecriture du bloc de queue dans le fichier
        aff_entete_L7OVC(fichier,3,entete_L7OVC(fichier,1)+1);// mise a jour du numero du bloc correspondant a la nouvelle queue dan sl'entete
        buf->suivant=-1;                         // mise a jour du suivant a nill
        sprintf(buf->tab,"%s","");               // vider la chaine du buffer
        ecriredir_L7OVC(fichier,entete_L7OVC(fichier,3),buf);// ecriture du buffer dans le bloc representatnt la nouvelle queue
        aff_entete_L7OVC(fichier,1,entete_L7OVC(fichier,1)+1);// incrementation du nombre de bloc alloues
}
//----------------------------------- // procedure qui permet d'ecrire n caractère du buffer----------------------------------------------//
void ecrire_chaine_enrg(L7OVC *fichier,int *i, int *j, char chaine[],Buffer_L7OVC *buf)
{
    ...
    for (k=0;k<n;k++)   // k pourn le deplacement dans la chaine
    {
        if ((*j)<=Taille_bloc_L7OVC)  //si je suis toujours dans le bloc conserne
        {
         ...
         {
             buf->tab[*j]='\0';
             ecriredir_L7OVC(fichier,*i,buf);  // ecriture du precedent buffer dans le fichier
             alloc_bloc_L7OVC(fichier);        // alocation d'un nouveau bloc afin de recevoir le reste de la chaine
...
int turn_string_to_enrg(enrg_L7OVC *A,char chaine[])
{
    if (chaine[0]!=0 && chaine[strlen(chaine)-1]=='&'){
        sscanf(chaine,"%ld$%d|%d|%d|%d|%d|%d|%d|%d",&(A->cle),
               &(A->Info.jour_Naissance),&(A->Info.mois_Naissance),&(A->Info.annee_Naissance),
               &(A->Info.Force_Armee),&(A->Info.Grade),&(A->Info.Groupe_Sanguin),&(A->Info.Region_Militaire),&(A->Info.Wilaya_Naissance));
...
void turn_enr_to_string(char chaine[],enrg_L7OVC *A)
{
    sprintf(chaine,"%d$%d|%d|%d|%d|%d|%d|%d|%d#%s#%s&",A->cle,
            A->Info.jour_Naissance,A->Info.mois_Naissance,A->Info.annee_Naissance,A->Info.Force_Armee,
            A->Info.Grade,A->Info.Groupe_Sanguin,A->Info.Region_Militaire,A->Info.Wilaya_Naissance,
            A->Info.Nom,A->Info.Prenom);
    //printf("%s",chaine);
...
void insert_Fin_L7OVC(L7OVC *fichier,enrg_L7OVC eng,int *i1,int *j1)
{
        ...
        if(entete_L7OVC(fichier,1)==0) // si la cle n'a  pas ete trouve alors on insere dans le premier bloc
            aff_entete_L7OVC(fichier,1,1);
        sprintf(chain,"%s","");
        i=entete_L7OVC(fichier,3); // positionnelent dans le queue
        j=entete_L7OVC(fichier,4);  // positionnnement a la position libre dans le bloc de queue
        liredir_L7OVC(fichier,i,&buf); // lecture du  bloc
        (*i1)=i;(*j1)=j;
        turn_enr_to_string(chain,&eng);
        ecrire_chaine_enrg(fichier,&i,&j,chain,&buf); // ecriture de la chaine construite
           // printf("\n\t%s\t",chain);
           // system("PAUSE");
        ecriredir_L7OVC(fichier,i,&buf); // ecriture du dernier bloc en cas de chevauchement
...
void recherche_L7OVC(L7OVC *fichier,long int cle,int *trouv,int *i, int *j )
{
    ...
    (*i)=entete_L7OVC(fichier,2);              // positionnement au bloc entete pour une recherche sequentielle
    (*j)=0;                              // première position
    if(entete_L7OVC(fichier,1)!=0)             // si le fichier nest pas vide
    {
        liredir_L7OVC(fichier,*i,&buf);     // lecture du premier bloc afin d'entammer le parcours

        while((stop==0) && ((*trouv)==0)) // tant qu'on pas trouve ou qu'on est pas a la fin du ficher
        {
            bloc=(*i);   // sauvegarde du bloc a partir duquel on a commence l'extraction de l'enregistrement
            pos=(*j);    //sauvegarde de la position
            recuperer_chaine_enrg(fichier,i,j,chaine,&buf);    // recuperation des champs
            turn_string_to_enrg(&Pr,chaine);
             if((Pr.cle==cle) && (Pr.eface==0)) // si l'info n'est pas efface logiquement et que la cle correspond
             {
                (*trouv)=1;   // arret de la recherche
            }
            else
            {
                if(((*i)==entete_L7OVC(fichier,3))&& ((*j)== entete_L7OVC(fichier,4))) // si on est a la fin du fichier on arrete
                {
                     stop=1;
                }
            }
        }
        (*i)=bloc; // renvoi du bloc et de la position aux quels no n a trouve l'info si elle existe
        (*j)=pos;
    }
...
void insertion_L7OVC(L7OVC *fichier,enrg_L7OVC Pr)
{
    ...
    recherche_L7OVC(fichier,Pr.cle,&trouv,&i,&j); // recherche de la cle por eviter les  dopublon
    if(trouv==0)
    {
...
void suppression_logique_L7OVC(L7OVC *fichier, long int cle)
{
    ...
    {
        liredir_L7OVC(fichier,i,&buf);   // lecture du bloc dans lequel on a trouve l'info
        recuperer_chaine_enrg(fichier,&i,&j,chaine,&buf); // recuperation de la chaine correpondant a la taille de l'info
        chaine[0]='1';
        ecrire_chaine_enrg(fichier,&i,&j,chaine,&buf); // ecriture de la chaine construite
        ecriredir_L7OVC(fichier,i,&buf); // ecriture du dernier bloc en cas de chevauchement
        aff_entete_L7OVC(fichier,5,entete_L7OVC(fichier,5)+strlen(chaine)); // mise az jour du nombre de caractère supprimes
...
void suppression_physique_L7OVC(L7OVC *fichier,char *nom_physique)
{
     ...
     bloc2=1;
     pos2=0;
     liredir_L7OVC(fichier,bloc1,&buf1);// lecture du premier bloc dans le fichier 1
     int stop=0;
     while(!stop)
     {
        sprintf(chaine,"%s",""); // recuperation des chaines
        recuperer_chaine_enrg(fichier,&bloc1,&pos1,chaine,&buf1);
        if(chaine[0]=='0') // sila cle n'a pas etet supprime logiquement on l'ensère dans le nouveau fichier
        {
             ecrire_chaine_enrg(fichier2,&bloc2,&pos2,chaine,&buf2);
             aff_entete_L7OVC(fichier2,3,bloc2);
             aff_entete_L7OVC(fichier2,4,pos2);
        }
        if((bloc1==entete_L7OVC(fichier,3))&&(pos1==entete_L7OVC(fichier,4))) // si onn est la fin du ficher source on  arrete
        {
            stop=1;
        }
     }

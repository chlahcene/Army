...
TnOF *ouvrirTnOF(char *nom_physique, char mode) {
  ...
  if ((mode == 'A') || (mode == 'a'))   // mode ancien
  {
    ...
    fread(&(fichier->entete_TnOF), sizeof(EnteteTnOF), 1,
...
void liredirTnOF(TnOF *f, int N_bloc, TblocTnOF *buffer) {
  ...
  if (N_bloc <= (f->entete_TnOF).nb_bloc) {
    fseek(f->fichier, sizeof(EnteteTnOF) + (N_bloc - 1) * sizeof(TblocTnOF),
          SEEK_SET);
...
int enteteTnOF(TnOF *f, int i) {
  if (i == 1)
    return (f->entete_TnOF).nb_bloc;
  if (i == 2)
    return (f->entete_TnOF).nb_enr;
  else
    return -1;
}
void aff_enteteTnOF(TnOF *f, int i, int val) {
  if (i == 1)
    (f->entete_TnOF).nb_bloc = val;
  if (i == 2)
    (f->entete_TnOF).nb_enr = val;
}

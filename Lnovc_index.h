    ...
    printf("\n entete nbr bloc %d \t nbr egr %d",enteteTOF(F,1),enteteTOF(F,2));
      ...
      for (i=1;i<=nb;i++){
        ...
        for (j=0;j<buf.nbenreg;j++){
            printf("\n%d : efface %d\t cle %d\t bloc %d \t indice_bloc %d",++y,buf.T[j].efface,buf.T[j].cle,buf.T[j].bloc,buf.T[j].indice_bloc);
        }
      }
}
...
void Reorganisation(L7OVC *F1,TOF *F2,char *Nom_Fichier,double u)
{
    ...
    TnOF *FY,*FF,*FG;EnregTnOF eng_tnof;
    ...
    for (i=1;i<=nb;i++){
        ...
        fclose(Table_Index);
    fermer_L7OVC(F);
    fermerTnOF(FY);fermerTnOF(FG);fermerTnOF(FF);
    fermer_TOF(FTOF);

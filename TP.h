...
char *GRADE[] = {"General de corps d'armee",
                 "General-Major",
                 "General",
                 "Colonel",
                 "Lieutenant-colonel",
                 ...
                 "Caporal",
                 "Djoundi"};
char *FORCE_ARMEE[] = {"Armee de terre",
                       "Armee de l'air",
                       "Marine nationale",
                       ...
                       "Sante militaire"};
char *REGION_MILITAIRE[] = {"1RM-Blida",   "2RM-Oran",        "3RM-Bechar",
                            "4RM-Ouargla", "5RM-Constantine", "6RMTamanrasset"};
void Affiche_ENG(enrg_L7OVC A) {
  ...
  textbackground(BLACK);
  gotoxy(i + 8, j + 4);
  printf("Matrucule :\t %ld", A.cle);
  gotoxy(i + 8, j + 6);
  printf("Nom :\t %s", A.Info.Nom);
  gotoxy(i + 8, j + 8);
  printf("Prenom :\t %s", A.Info.Prenom);
  gotoxy(i + 8, j + 10);
  printf("Date Naissance :\t %d/%d/%d", A.Info.jour_Naissance,
         A.Info.mois_Naissance, A.Info.annee_Naissance);
  gotoxy(i + 8, j + 12);
  printf("Wilaya Naissance :\t %s", WILAYA[A.Info.Wilaya_Naissance - 1]);
  gotoxy(i + 8, j + 14);
  printf("Groupe Sanguin :\t %s", GROUPE_SANGUIN[A.Info.Groupe_Sanguin - 1]);
  gotoxy(i + 8, j + 16);
  printf("Grade :\t %s", GRADE[A.Info.Grade - 1]);
  gotoxy(i + 8, j + 18);
  printf("Force Armee :\t %s", FORCE_ARMEE[A.Info.Force_Armee - 1]);
  gotoxy(i + 8, j + 20);
  printf("Region Militaire :\t %s",
         REGION_MILITAIRE[A.Info.Region_Militaire - 1]);
  gotoxy(i + 8, j + 22);
  system("PAUSE");
}
...
int Nom_Fichier(char **nom) {
  // affiche liste des fichiers et l'utilisatuer choiser un nom fichier pour
  // l'utiliser
  char **NFile;
  int i, j, taille;
...
int lire_ENG(enrg_L7OVC *A) {
  ...
  int NumbreInput = 1;
  int c = 0;
  char Matriculle[48] = "\0", LastName[48] = "\0", FirstName[48] = "\0",
       ch[100];
  ...
  do {
    if (NumbreInput > 0 && NumbreInput < 10) {
      ...
      if (NumbreInput == 1) {
        ...
        // Date naissance
        c = Input_Date(&(A->Info.jour_Naissance), &(A->Info.mois_Naissance),
                       &(A->Info.annee_Naissance), x, y + 27);

      } else if (NumbreInput == 5) {
        // Wilaya naissance
        A->Info.Wilaya_Naissance = Menu2(WILAYA, 48, x, y + 33, 48, 1, WHITE,
                                         WHITE, A->Info.Wilaya_Naissance);

      } else if (NumbreInput == 6) {
        // Groupe Sanguin
        A->Info.Groupe_Sanguin =
            Menu2_sans(GROUPE_SANGUIN, 8, x, y + 39, 48, 1, WHITE, WHITE,
                       A->Info.Groupe_Sanguin);

      } else if (NumbreInput == 7) {
        // Grade
        A->Info.Grade = Menu2_sans(GRADE, 17, x, y + 45, 48, 1, WHITE, WHITE,
                                   A->Info.Grade);

      } else if (NumbreInput == 8) {
        // Force Armee
        A->Info.Force_Armee = Menu2_sans(FORCE_ARMEE, 8, x, y + 51, 48, 1,
                                         WHITE, WHITE, A->Info.Force_Armee);

      } else if (NumbreInput == 9) {
        // Region Militaire
        A->Info.Region_Militaire =
            Menu2_sans(REGION_MILITAIRE, 6, x, y + 57, 48, 1, WHITE, WHITE,
                       A->Info.Region_Militaire);
      }
      if (c == 80 || c == 81 || c == 9 || c == 13)
        NumbreInput++;
      ...
      do {
        c = SaiserCntrl();
               ...
               c == 77); // car here touch left and right ne fait rien
      if (c == 13) {
        ...
        if (A->Info.annee_Naissance < 1940 || A->Info.annee_Naissance > 1999) {
          // This is input vide
          AfficheErreur(x + 5, y + 5,
                        "La date naissance doit entre entre [1940 .. 1999]");
          textcolor(BLACK);
          textbackground(WHITE);
          continue;
        }
        ...
        if (A->cle < 111111 || A->cle > 999999) {
          // This is input vide
          AfficheErreur(x + 5, y + 5,
                        "Le matricule doit entre entre [111111 .. 999999]");
          textcolor(BLACK);
          textbackground(WHITE);
          continue;
        }

...
char* TimeActuelle(void){
    ...
    sprintf(Tm,"%04d%02d%02d%02d%02d%02d",TA->tm_year+1900,TA->tm_mon+1,TA->tm_mday,TA->tm_hour,TA->tm_min,TA->tm_sec);
    return Tm;
}
char** Texte_split (const char *s, const char *ct,int NumberMax){
...
void File_Derectory_Current(char ***Fichier,int *Taille){
    // vous donne le name des fichier qui se trouve dans le dossier current
    // il fait le alloucation alors il dois faire liberation
    system("dir /b /a /o:gn  > d:\\file_tmp");
    FILE* F=fopen("d:\\file_tmp","r");
    if (F!=NULL){
        int i,j;(*Taille)=1;
        char tab[1000];
        ...
        fseek(F,0,SEEK_SET);
        i=0;
        (*Fichier)[i] = malloc(3*sizeof(char));
        strcpy((*Fichier)[i++],"..");
        while(fgets(tab,1000,F)!=NULL){
             (*Fichier)[i]=malloc((strlen(tab)+1)*sizeof(char));
              tab[strlen(tab)-1]=0;
              strcpy((*Fichier)[i++],tab);
        }
        fclose(F);
        remove("d:\\file_tmp");
    }else {
        printf("Erruer d'ouverture fichier");
    }
...
void** Alouer_2D(char type,int NbrLign,int Nbrcln){
    int i;
    void** K=NULL;
    if (type=='i'){
        int** M=(int**)malloc(sizeof(int*)*NbrLign);
        for (i=0;i<NbrLign;i++){
             M[i]=(int*)malloc(sizeof(int)*Nbrcln);
        }
        return M;
    }else if (type=='c'){
        char** C=(char**)malloc(sizeof(char*)*NbrLign);
        for (i=0;i<NbrLign;i++){
             C[i]=(char*)malloc(sizeof(char)*Nbrcln);
        }
        return C;
    }

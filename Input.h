...
int Input_MotPass(char *MotPass, int CooredonneX, int CooredonneY,
                  int maxlength);
// Defines a single-line Mot Pass and save in Number
int Input_Area_Number(char *Number, int CooredonneX, int CooredonneY,
                      int maxlength, int maxLength_Line);
...
// print the string chain in the screen in position x and y with pause Temp ms
// between All char
...
int Menu(const char **Ph, int NbrPhrase, int x, int y, int Longueur,
         int Hauteur, int colorbackground, int colorMenu);
// Affichez Le menu return le nemero choiser return 0 c'est choiser retourner en
// arreire
int Menu_sans(const char **Ph, int NbrPhrase, int x, int y, int Longueur,
              int Hauteur, int colorbackground, int colorMenu);
// Affichez Le menu return le nemero choiser return 0 c'est choiser retourner en
// arreire sans
int Menu2(const char **Ph, int NbrPhrase, int x, int y, int Longueur,
          int Hauteur, int colorbackground, int colorMenu, int select_nemero);
// Affichez Le menu return le nemero choiser return select_nemero c'est choiser
// retourner en arreire
int Menu2_sans(const char **Ph, int NbrPhrase, int x, int y, int Longueur,
               int Hauteur, int colorbackground, int colorMenu,
               int select_nemero);
// Affichez Le menu return le nemero choiser return select_nemero c'est choiser
// retourner en arreire
void AffichezListeChoix(const char **ch, int NbrChoix, int num,
                        int MaxLongueurString, int x, int y);
// Affichez un liste des chiox qui se trouve dans tableau ch de Nbrchoix phrase
void AffichezListeChoix_sans(const char **ch, int NbrChoix, int num,
                             int MaxLongueurString, int x, int y);
// Affichez un liste des chiox qui se trouve dans tableau ch de Nbrchoix phrase
int SaiserCtrlText(void);
// attendre que utilisateur taper les touch du controlle text comme cntrl F ..
  ...
  do {
    ...
    if (toucheClavier == 0 || toucheClavier == 224) {
      ...
      if (toucheClavier == 224 && toucheClavier1 == 75) {
        ...
        if (i > 0)
          i--; // on decremante la postiion du la curseur
      } else if (toucheClavier == 224 && toucheClavier1 == 77) {
        // touch clavier est touch right
        if (i < TailleDuTexte)
          i++; // on incremante la postiion du la curseur
        ...
        if (i < TailleDuTexte) {
          ...
          printf("%c", 7);
      } else if (toucheClavier == 224 &&
                 (toucheClavier1 == 73 || toucheClavier1 == 81 ||
                  toucheClavier1 == 80 || toucheClavier1 == 72)) {
        // touch clavier est touch up ou down ou page suivant ou precedant
        return toucheClavier1; // on sort du la fonction avec return du le touch
                               // qui fait sortire
      }
      ...
      if (toucheClavier == 8) {
        ...
        if (i > 0) {
          ...
          printf("%c", 7);
      } else if (toucheClavier == 9 || toucheClavier == 27 ||
                 toucheClavier == 13) {
        // si les touch entres ou echap ou -- on sort de la fonction
        return toucheClavier; // on sort du la fonction avec return du le touch
                              // qui fait sortire
      }
    } else if (isprint(toucheClavier) || isCharSpFr(toucheClavier)) {
      if (TailleDuTexte < maxlength) {
        StrInsertChar(
            text, (char)toucheClavier,
            i++); // on Insert le cractere toucheClavier a la postion i
...
int Input_radio(int Cooredonne[][max_input_radio], int choix[][max_input_radio],
                int taille) {
  ...
  while (taille > 0) {
    ...
    if (toucheClavier == 0 || toucheClavier == 224) {
      ...
      if (toucheClavier == 224 && toucheClavier1 == 75) {
        ...
        if (j > 0)
          j--; // on decremante la postiion du la curseur
      } else if (toucheClavier == 224 && toucheClavier1 == 77) {
        // touch clavier est touch right
        if (j < Cooredonne[i][1] - 1)
          j++; // on incremante la postiion du la curseur
      } else if (toucheClavier == 224 && toucheClavier1 == 79) {
        // touch clavier est touch Fin de Line
        ...
        if (toucheClavier1 == 81 || toucheClavier1 == 80) {
          if (i == taille - 1)
            return toucheClavier1; // on sort du la fonction avec return du le
                                   // touch qui fait sortire
          i++;
...
int Input_Number(char *Number, int CooredonneX, int CooredonneY,
                 int maxlength) {
  ...
  do {
    ...
    if (toucheClavier == 0 || toucheClavier == 224) {
      ...
      if (toucheClavier == 224) {
        if (toucheClavier1 == 75 || toucheClavier1 == 77 ||
            toucheClavier1 == 73 || toucheClavier1 == 81 ||
            toucheClavier1 == 80 || toucheClavier1 == 72) {
  ...
  do {
    ...
    if (toucheClavier == 0 || toucheClavier == 224) {
      ...
      if (toucheClavier == 8) {
        ...
        if (i > 0) {
          StrSuppChar(MotPass,
                      --i); // on supprime le cractere de la postion i-1
          text[--TailleDuTexte] = '\0';
          cputsxyBlanc(CooredonneX, CooredonneY, text, TailleDuTexte + 1);
        } else
          printf("%c", 7);
...
void AfficheButton(const char *NomButton, int x, int y, int length,
                   int height) {
  /// display the button in the screen
...
void cputsxyTemp(int x, int y, const char *chain, int Temp) {
  /// print the string chain in the screen in position x and y with pause Temp
  /// ms between All char
...
int Menu(const char **Ph, int NbrPhrase, int x, int y, int Longueur,
  ...
  AffichezListeChoix(Ph + 1, Mn - 1, 2, Longueur, x,
                     y + 1); /// affichez le reste des choix avec le Normal
...
int Menu_sans(const char **Ph, int NbrPhrase, int x, int y, int Longueur,
  ...
  textbackground(colorbackground);
  DrawRectangle(x, y, Longueur, Hauteur);
  AffichezListeChoix_sans(Ph + 1, Mn - 1, 2, Longueur, x,
                          y + 1); /// affichez le reste des choix avec le Normal
  textbackground(colorMenu);      /// Affichez le prmier choix avec le colorMenu
  ...
  do {
    ...
    if (c == 72) {
      ...
      if ((f == 0) && (NbrPhrase > Mn)) {
        if (chx == 1) {
          f = Mn - 1;
          AffichezListeChoix_sans(&Ph[NbrPhrase - Mn], Mn - 1,
                                  NbrPhrase - Mn + 1, Longueur, x, y);
        } else
          AffichezListeChoix_sans(&Ph[chx - 1], Mn - 1, chx, Longueur, x,
                                  y + 1);
      } else {
      ...
      if ((f == Mn - 1) && (NbrPhrase > Mn)) {
        if (chx == NbrPhrase) {
          ...
          AffichezListeChoix_sans(&Ph[chx - f], Mn - 1, chx - f + 1, Longueur,
                                  x, y);
      } else {
        AffichezListeChoix_sans(&Ph[chx - 1], 1, chx, Longueur, x, y + f);
        f = ((f + Mn) + 1) % Mn; /// incrimanter le f mais il dois restez sur
    ...
    if (toucheClavier == 0 || toucheClavier == 224) {
      ...
      if (toucheClavier == 224) {
        if (toucheClavier1 == 75 || toucheClavier1 == 77 ||
            toucheClavier1 == 73 || toucheClavier1 == 81 ||
            toucheClavier1 == 80 || toucheClavier1 == 72 ||
            toucheClavier1 == 71 || toucheClavier1 == 79 ||
            toucheClavier1 == 61) {
          return toucheClavier1;
        }
      }
    } else if (toucheClavier == 9 || toucheClavier == 27 ||
               toucheClavier == 13 || toucheClavier == 8 ||
               toucheClavier == 6 || toucheClavier == 32) {
      // si les touch entres ou echap ou -- on sort de la fonction
      return toucheClavier;
    }
  } while (1);
...
int Input_Area_Number(char *Number, int CooredonneX, int CooredonneY,
  ...
  int NbrChiffre = strlen(Number);
  int i = NbrChiffre;
  cputsxyAreaBlanc(CooredonneX, CooredonneY, Number,
                   maxlength / maxLength_Line + 1, maxLength_Line,
                   "0123456789");
  do {
    ...
    if (toucheClavier == 0 || toucheClavier == 224) {
      ...
      if (toucheClavier == 224 && toucheClavier1 == 75) {
        ...
        if (i < NbrChiffre) {
          StrSuppChar(Number, i); // on supprime le cractere de la postion i
          cputsxyAreaBlanc(CooredonneX, CooredonneY, Number,
                           NbrChiffre / maxLength_Line + 1, maxLength_Line,
                           "0123456789");
          NbrChiffre--;
        } else
          printf("%c", 7);
      ...
      if (NbrChiffre < maxlength) {
        StrInsertChar(
            Number, (char)toucheClavier,
            i++); // on Insert le cractere toucheClavier a la postion i
        cputsxyArea(CooredonneX, CooredonneY, Number, maxLength_Line,
                    "0123456789");
        NbrChiffre++;
      } else
        printf("%c", 7);
...
int Input_Date(int *jour, int *mois, int *annee, int x, int y) {
  ...
  char *Les_Mois[] = {"janvier",   "fevrier", "mars",     "avril",
    ...
    if (NumbreInput > 0 && NumbreInput < 4) {
      ...
      if (c == 13 || c == 27 || (c == 80 || c == 81 || c == 72 || c == 73)) {
        ...
        if (j > maxjour(a, m) || j < 1) {
          char ch[100];
          sprintf(ch, "Le Date %02d/%02d n'existe pas cette mois a %d jours ",
                  j, m, maxjour(a, m));

...
void recharche_TnOF(TnOF *F,long int cle,int *bloc,int *cell,int *trouve)
{
    /*
    (*trouve)=0;
    int i,j;
    TblocTnOF buf;
    int stop=0,nbr_bloc=enteteTnOF(F,1),nbr_eng;
    for (i=0;i<nbr_bloc && stop==0;i++){
        liredirTnOF(F,i+1,&buf);
        nbr_eng=enteteTnOF(F,2);
        for (j=0;j<nbr_eng;j++){
            if (buf.T[j].cle==cle && buf.T[j].efface==0){
                stop=1;
                (*trouve)=1;
                (*bloc)=i+1;
                (*cell)=j;
...
int supp_logique_TnOF(TnOF *F,long int cle)
{
    int i,j,trouve;
    TblocTnOF buf;
    recharche_TnOF(F,cle,&i,&j,&trouve);
    if (trouve==1){
        liredirTnOF(F,i,&buf);
        buf.T[j].efface=1;
        ecriredirTnOF(F,i,&buf);
        return 1;
    }else return 0;
}

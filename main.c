...
void ChioxMenu() {
  int i;
  int a = 3, s = 3;
  char *S[] = {"Creat Fichier", "consuleter fichier ", "Quitter Le Programme"};
  char *A[] = {"Ajoute Nouveau", "Recharche, consuleter, modifier, supprimer",
               "Stat et structure"};
        ...
        if (Nom_Fichier(&nom_fichier)) {
          ...
          do {
            ...
            if (i == 1) {
              ...
              printf(
                  "\t numero du bloc representatnt la tete du fichier : %d \n",
                  entete_L7OVC(F_lovc, 2));
              printf(
                  "\t numero du bloc representatnt la tete du ficheir : %d \n",
              ...
              printf("\n\t Fichier Principale : \t le nombre lecteur %ld \t "
                     "ecriteur %ld\n",
                     ecrire_stat_lonvc, lire_stat_lonvc);
              printf("\n\t Index Primaire : \t le nombre lecteur %ld \t "
                     "ecriteur %ld\n",
                     ecrire_stat_tof, lire_stat_tof);
              printf("\n\t Index Secondaire : \t le nombre lecteur %ld \t "
                     "ecriteur %ld\n",
